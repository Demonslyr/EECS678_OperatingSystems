...
int fileLength = statbuf.st_size;
  /* 
   * 2. go to the location corresponding to the last byte (of the output file)
   */
lseek(fdout,fileLength-1,SEEK_SET);
...
write(fdout,"",1);
   ...
   */
src = mmap(NULL,fileLength, PROT_READ, MAP_SHARED, fdin,0);
  /* 
   * 5. mmap the output file 
   */
dst = mmap(NULL,fileLength, PROT_READ | PROT_WRITE, MAP_SHARED, fdout,0);
  /* 
